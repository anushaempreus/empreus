  ...
  return (
    <>
      ...
      <section style={{ padding:"clamp(48px,6vw,78px) clamp(16px,3.5vw,48px)", textAlign:"center", borderTop:"1px solid var(--rule-soft)", borderBottom:"1px solid var(--rule-soft)" }}>
        ...
        <form onSubmit={e=>e.preventDefault()} style={{ display:"flex", maxWidth:480, margin:"0 auto", borderBottom:"1px solid var(--ink)" }}>
          <input type="email" placeholder="your@email.com" required style={{ flex:1, border:"none", background:"transparent", padding:"12px 4px", fontFamily:"inherit", fontSize:14, color:"var(--ink)", outline:"none" }} />
          <button type="submit" style={{ background:"none", border:"none", color:"var(--ink)", fontSize:12, fontWeight:500, letterSpacing:".15em", textTransform:"uppercase", padding:"12px 4px", transition:"color .2s", cursor:"pointer" }}
        ...
        <div style={{ maxWidth:"var(--max)", margin:"0 auto", display:"grid", gridTemplateColumns:"1.5fr repeat(4,1fr)", gap:"clamp(24px,3vw,56px)", paddingBottom:54, borderBottom:"1px solid var(--rule)" }} className="foot-grid">
          ...
          <div>
            ...
            <ul style={{ listStyle:"none", padding:0 }}>
              {[["tel:0261891322","02 6189 1322"],["mailto:support@empreus.com.au","support@empreus.com.au"]].map(([href,label])=>(
